...
# async def is_dj_check(ctx):
#     guild = ctx.guild
#     author = ctx.author
#     if await r.table("music").get(str(guild.id)).get_field("dj_role").run(ctx.bot.conn) is None:
#         return True
#     dj_id = await r.table("music").get(str(guild.id)).get_field("dj_role").run(ctx.bot.conn)
#     dj_role = discord.utils.get(guild.roles, id=int(dj_id))
